...
public class Main {
    ...
    private static void adicionarImoveisIniciais() {
        if (imoveis.isEmpty()) {
            imoveis.add(new Imovel("Casa da Colina", 3000, 500));
            imoveis.add(new Imovel("Apartamento Azul", 4500, 650));
            imoveis.add(new Imovel("Loja da Esquina", 6000, 800));
            imoveis.add(new Imovel("Sítio Palmeira", 10000, 1500));
            imoveis.add(new Imovel("Hotel Central", 12000, 1800));
            imoveis.add(new Imovel("Fazenda Boa Vista", 15000, 2000));
            imoveis.add(new Imovel("Condomínio Luxo", 8000, 1200));
            imoveis.add(new Imovel("Estúdio Criativo", 5000, 700));
            imoveis.add(new Imovel("Galpão Industrial", 9500, 1400));
            imoveis.add(new Imovel("Cobertura Panorâmica", 11000, 1600));
        }
    }
    ...
    private static void iniciarJogo(Scanner scanner) {
        ...
        while (rodadaAtual <= maxRodadas) {
            ...
            while (iter.hasNext()) {
                ...
                if (jogador.getSaldo() <= 0) {
                    System.out.printf("Jogador %s desistiu e foi removido do jogo.%n", jogador.getNome());
                    iter.remove(); // remove com segurança durante iteração
                }
            }
